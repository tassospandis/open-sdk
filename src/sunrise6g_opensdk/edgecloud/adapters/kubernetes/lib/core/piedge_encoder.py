...
def deploy_service_function(
    ...
    deployed_name = auxiliary_functions.prepare_name(
        service_function.service_function_instance_name, driver
    )

    final_deploy_descriptor = _build_deploy_descriptor(
        service_function, ser_function_[0], deployed_name
    )
    if isinstance(final_deploy_descriptor, tuple):
        return final_deploy_descriptor  # error message and code

    response = kubernetes_connector.deploy_service_function(final_deploy_descriptor)

    deployed_service_function_db = _build_deployed_service_function_db(
        service_function, ser_function_[0], deployed_name, final_deploy_descriptor
    )
...
def _build_deploy_descriptor(service_function, ser_function, deployed_name):
    ...
    containers_result = _build_containers(service_function, ser_function)
    if isinstance(containers_result, tuple):
        return containers_result  # error message and code
    final_deploy_descriptor["containers"] = containers_result

    volumes_result = _build_volumes(service_function, ser_function)
    if isinstance(volumes_result, tuple):
        return volumes_result  # error message and code
    if volumes_result:
        final_deploy_descriptor["volumes"] = volumes_result

    env_result = _build_env_parameters(service_function, ser_function)
    if isinstance(env_result, tuple):
        return env_result  # error message and code
    if env_result:
        final_deploy_descriptor["env_parameters"] = env_result

    return final_deploy_descriptor


def _build_containers(service_function, ser_function):
    containers = []
    con_ = {}
    con_["image"] = ser_function["image"]
    if "privileged" in ser_function:
        con_["privileged"] = ser_function["privileged"]
    application_ports = ser_function["application_ports"]
...
def _build_volumes(service_function, ser_function):
    req_volumes = []
    if "required_volumes" in ser_function:
        if ser_function["required_volumes"] is not None:
            for required_volumes in ser_function["required_volumes"]:
    ...
    if len(vol_mount) != len(req_volumes):
        ...
        if ser_function.get("required_volumes") is not None:
            ...
            if result is False:
                ...
                volumes = []
                for vol in ser_function["required_volumes"]:
...
def _build_env_parameters(service_function, ser_function):
    req_env_parameters = []
    if "required_env_parameters" in ser_function:
        if ser_function["required_env_parameters"] is not None:
            for required_env_parameters in ser_function["required_env_parameters"]:
    ...
    if len(env_names) != len(req_env_parameters):
        ...
        if ser_function.get("required_env_parameters") is not None:
            ...
            if result is False:
                ...
                paremeters = []
                for reqenv in ser_function.get("required_env_parameters"):
...
def _build_deployed_service_function_db(
    service_function, ser_function, deployed_name, final_deploy_descriptor
):
    deployed_service_function_db = {}
    deployed_service_function_db["service_function_name"] = ser_function["name"]
    if service_function.location is not None:
    ...
    return deployed_service_function_db
