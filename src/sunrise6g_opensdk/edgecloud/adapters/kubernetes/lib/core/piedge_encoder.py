...
def deploy_service_function(
    ...
    #     if not node_:
    #         return "The given location does not exist in the node catalogue", 404
    ...
    if service_function.all_node_ports is not None:
        ...
        if service_function.all_node_ports==False and service_function.node_ports is None:
            return "Please provide the application ports in the field exposed_ports or all_node_ports==true", 400
   ...
   #check volumes!!
    ...
    if (len(vol_mount) != len(req_volumes)):
        return "The selected service function requires " + str(len(req_volumes)) +" volume/ volumes ", 400
    else:
        if ser_function_[0].get("required_volumes") is not None:
            ...
            if result is False:
                return "The selected service function requires " + str(len(req_volumes)) +" volumes. Please check volume names", 400
            else:
                volumes=[]
    ...
    if (len(env_names) != len(req_env_parameters)):
        return "The selected service function requires " + str(len(req_env_parameters)) + " env parameters", 400
    else:
        if ser_function_[0].get("required_env_parameters") is not None:
            ...
            if result is False:
                return "The selected service function requires " + str(
                    len(req_env_parameters)) + " env parameters. Please check names of env parameters", 400
            else:
