...
class TestK8sEdgeApplicationManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        TOKEN = os.environ.get(
            "NEW_TOKEN"
        )  # Use environment variable if set, else None
        ...
        if TOKEN is not None:
            ...
            print(
                "Environment variable 'NEW_TOKEN' is not set. Using TOKEN set in test_k8s_client.py"
            )
    ...
    def test_02_onboard_deploy(self):
        ...
        deployment_zones = [
            {"EdgeCloudZone": {"edgeCloudZoneName": deploymentlocation}}
        ]  # Set the deployment zone
        print(f"--> Step 2: Deploying app '{app_name}' to zone 'minikube'...")
        deploy_response = self.manager.deploy_app(
            app_id=app_id, app_zones=deployment_zones
        )
        self.assertIsNotNone(
            deploy_response, "Deploy command did not return a response."
        )
